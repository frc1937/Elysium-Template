...
public class AdvancedSwerveKinematics {
    ...
    public static ChassisSpeeds correctForDynamics(ChassisSpeeds requestedSpeeds) {
        Pose2d futureRobotPose = new Pose2d(
                requestedSpeeds.vxMetersPerSecond * ROBOT_PERIODIC_LOOP_TIME,
                requestedSpeeds.vyMetersPerSecond * ROBOT_PERIODIC_LOOP_TIME,
                Rotation2d.fromRadians(requestedSpeeds.omegaRadiansPerSecond * ROBOT_PERIODIC_LOOP_TIME)
        );

        Twist2d twistForPose = log(futureRobotPose);

        return new ChassisSpeeds(
                twistForPose.dx / ROBOT_PERIODIC_LOOP_TIME,
                twistForPose.dy / ROBOT_PERIODIC_LOOP_TIME,
                twistForPose.dtheta / ROBOT_PERIODIC_LOOP_TIME
        );
    }
    ...
    public SwerveModuleState[] toSwerveModuleStates(ChassisSpeeds desiredSpeed, Rotation2d robotHeading, RotationFrame rotationFrame) {
        ...
        SwerveModuleState[] swerveModuleStates = new SwerveModuleState[moduleLocations.length];
        double[] moduleTurnSpeeds = new double[moduleLocations.length];
