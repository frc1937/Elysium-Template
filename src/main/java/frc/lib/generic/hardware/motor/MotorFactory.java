...
public class MotorFactory {
    private static final List<SimulatedMotor> REGISTERED_SIMULATIONS = new ArrayList<>();
    ...
    private static Motor createSimOrReplayMotor(String name, int port) {
        ...
        if (CURRENT_MODE == GlobalConstants.Mode.SIMULATION) {
            final SimulatedMotor simulation = new SimulatedMotor(name, port);
    ...
    public static void updateAllSimulations() {
        for (SimulatedMotor simulation : REGISTERED_SIMULATIONS) {
